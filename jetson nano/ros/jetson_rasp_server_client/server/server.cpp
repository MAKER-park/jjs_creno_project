...
typedef struct {
	int index;		number(connectednumber, noconnected0)
...
int main(int argc, char argv[])
{
	...
	char idpasswd[(ID_SIZE2)+2];		input id, password
	...
	CLIENT_INFO client_info[MAX_CLNT] =
		{{0,-1,,jetson,jetson}, {0,-1,,rasp,rasp}};
	...
	while(1)
	{
		...
		{
			fputs(socket fulln, stdout);
			sprintf(msg,[%s] Socket full!n,pArray[0]);
		...
		{
			sprintf(msg,[%s] Try login!n,pArray[0]);
			write(clnt_sock, msg,strlen(msg));
		}
			...
			for(i = 0; i  MAX_CLNT; i++){
				if(!strcmp(client_info[i].id,pArray[0])){	compare id
					...
					if(!strcmp(client_info[i].pw, pArray[1])){	compare password
						...
						client_info[i].index = clnt_cnt;
						client_info[i].fd = clnt_sock;
						if(clnt_cnt == 0) first_client_info = &client_info[i];
						clnt_cnt++;
						pthread_mutex_unlock(&mutx);

						sprintf(msg,[%s] New connected! (ip%s,fd%d,sockcnt%d)n,
...
void handle_clnt(void arg)
{
	...
	sprintf(strBuff,Disconnect ID%s (ip%s,fd%d,sockcnt%d)n,client_info-id,client_info-ip,client_info-fd,clnt_cnt-1);
	fputs(strBuff, stdout);

	pthread_mutex_lock(&mutx);
	if(client_info-fd == first_client_info-fd && clnt_cnt1)
		first_client_info++;
	clnt_cnt--;
	client_info-fd = -1;
	pthread_mutex_unlock(&mutx);
...
void send_msg(MSG_INFO msg_info, CLIENT_INFO first_client_info)
{
	...
	{	
		char strBuff[20];
		for(i=0; iMAX_CLNT; i++){
			if((int)(msg_info-fd) == (first_client_info+i)-fd) continue;
