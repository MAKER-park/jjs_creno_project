...
void move_cb(const std_msgs::String cmd_msg) { //좌표값을 파싱으로 x축, y축 값에따라 크레인 이동
  if (flag == READY) {
    Split(cmd_msg.data, ',');
...
//ros를 사용하기 위한 명령어
ros::Subscriber<std_msgs::String> sub1("move", move_cb);
ros::Subscriber<std_msgs::Empty> sub2("home", home_cb);
